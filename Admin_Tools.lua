...
function drawSpectateMenu()
	...
	if recInfo.loading or not result then
		...
		if imgui.Button(u8'Check AFK', imgui.ImVec2(100, 20)) then
		...
		imgui.SameLine()
		if imgui.Button(u8'/aheal', imgui.ImVec2(73, 20)) then
		...
		if imgui.Button("REFRESH", imgui.ImVec2(300,25)) then
			sampSendClickTextdraw(scriptInfo.textdraws.refreshId)
		end

		if imgui.Button("STOP", imgui.ImVec2(300,25)) then
			sampSendChat("/re off")
		end
	end
...
function nameTagOn()
	...
	NTdist = memory.getfloat(pStSet + 39) -- РґР°Р»СЊРЅРѕСЃС‚СЊ
	NTwalls = memory.getint8(pStSet + 47) -- РІРёРґРёРјРѕСЃС‚СЊ С‡РµСЂРµР· СЃС‚РµРЅС‹
	NTshow = memory.getint8(pStSet + 56) -- РІРёРґРёРјРѕСЃС‚СЊ С‚РµРіРѕРІ
...
function rpc_init()
	...
	function sampev.onShowDialog(dialogId, style, title, button1, button2, text)
		if dialogId ~= 65535 and title:find("Ввод пароля") and mainIni.settings.autologin and mainIni.settings.password ~= "" then
